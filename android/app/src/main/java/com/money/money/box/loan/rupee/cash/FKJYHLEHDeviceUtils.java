...
public class FKJYHLEHDeviceUtils {
...
//                jsonObject.put("photo_id", photo_id);
    ...
    private static U getOtherData(Activity context) {
        ...
        List<String> sysPhotos = FKJYHLEHPDriverInfoUtil.getSystemPhotoList(context);
        boolean isRoot = FKJYHLEHPDriverInfoUtil.isRooted();
        boolean isEmulator = FKJYHLEHPDriverInfoUtil.isEmulator();
        ...
        u.add("keyboard", isNullText(FKJYHLEHPDriverInfoUtil.getKeyboard(context))); //__xor__
        // SMT_RANDOM_SORT_END
        try {
            // SMT_RANDOM_SORT_BEGIN
            u.add("cpuNumber", FKJYHLEHPDriverInfoUtil.getCpuNumCores()); //__xor__
            u.add("appMaxMemory", FKJYHLEHPDriverInfoUtil.getMemory(context)[0]); //__xor__
            u.add("appAvailableMemory", FKJYHLEHPDriverInfoUtil.getMemory(context)[1]); //__xor__
            u.add("appFreeMemory", FKJYHLEHPDriverInfoUtil.getMemory(context)[2]); //__xor__
            u.add("totalBootTime", FKJYHLEHPDriverInfoUtil.getOsTime(context)[0]); //__xor__
            u.add("totalBootTimeWake", FKJYHLEHPDriverInfoUtil.getOsTime(context)[1]); //__xor__
            u.add("maxBattery", FKJYHLEHPDriverInfoUtil.getBattery(context)[0]); //__xor__
            u.add("levelBattery", FKJYHLEHPDriverInfoUtil.getBattery(context)[1]); //__xor__
            // SMT_RANDOM_SORT_END
        } catch (Exception e) {

        }
        try {
            Object dbm = FKJYHLEHPDriverInfoUtil.getCellInfo(context).get("dbm"); //__xor__
            if (dbm != null) {
                u.add("dbm", isNullText(dbm.toString())); //__xor__
            }
        } catch (Exception e) {
        }
        u.add("lastBootTime", FKJYHLEHPDriverInfoUtil.getLastBootTime(context)); //__xor__
        try {
...
//            FlutterStartActivity.Companion.getInstance().getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
    ...
    private static U getStorage(Activity context) {
        ...
        try {
            // SMT_RANDOM_SORT_BEGIN
            u.add("ramTotalSize", isNullText(FKJYHLEHPDriverInfoUtil.getRamTotalSize(context))); //__xor__
            u.add("ramUsableSize", isNullText(FKJYHLEHPDriverInfoUtil.getRamAvailSize(context))); //__xor__
            u.add("mainStorage", isNullText(FKJYHLEHPDriverInfoUtil.getRootDirectory())); //__xor__
            u.add("externalStorage", isNullText(FKJYHLEHPDriverInfoUtil.getExternalStorageDirectory())); //__xor__
            u.add("memoryCardSize", isNullText(FKJYHLEHPDriverInfoUtil.getSDInfo().get("totalSize").toString())); //__xor__
            u.add("memoryCardSizeUse", isNullText(FKJYHLEHPDriverInfoUtil.getSDInfo().get("useSize").toString())); //__xor__
            u.add("internalStorageTotal", isNullText(FKJYHLEHPDriverInfoUtil.getTotalInternalMemorySize() + ""));
            u.add("internalStorageUsable", isNullText(FKJYHLEHPDriverInfoUtil.getAvailableInternalMemorySize() + ""));
            // SMT_RANDOM_SORT_END
        } catch (Exception e) {
        }
        return u;
    ...
    private static U getSimCard(Activity context) {
        U u = U.init();
        // SMT_RANDOM_SORT_BEGIN
        u.add("countryIso", FKJYHLEHPDriverInfoUtil.getSimCountryIso(context)); //__xor__
        u.add("serialNumber", isNullText(FKJYHLEHPDriverInfoUtil.getSerialNumber())); //__xor__
        u.add("simCardReady", FKJYHLEHPDriverInfoUtil.isSimCardReady(context)); //__xor__
        // SMT_RANDOM_SORT_END

        // SMT_RANDOM_SORT_BEGIN
        u.add("mobileData", FKJYHLEHPDriverInfoUtil.isMobileData(context)); //__xor__
        u.add("dataNetworkType", FKJYHLEHPDriverInfoUtil.getDataNetworkType(context)); //__xor__
        // SMT_RANDOM_SORT_END
        try {
    ...
    private static U getHardware(Activity context) {
        U u = U.init();
        u.add("deviceName", FKJYHLEHPDriverInfoUtil.getDriverDevice()) //__xor__
                // SMT_RANDOM_SORT_BEGIN
                .add("brand", FKJYHLEHPDriverInfoUtil.getBrand()) //__xor__
                .add("product", FKJYHLEHPDriverInfoUtil.getDriverProduct()) //__xor__
                .add("model", FKJYHLEHPDriverInfoUtil.getModel()) //__xor__
                .add("release", FKJYHLEHPDriverInfoUtil.getDriverOsVersion()) //__xor__
                .add("cpuType", FKJYHLEHPDriverInfoUtil.getCpuName()) //__xor__
                .add("sdkVersion", FKJYHLEHPDriverInfoUtil.getDriverSDKVersion()) //__xor__
                .add("serialNumber", FKJYHLEHPDriverInfoUtil.getSerialNumber()) //__xor__
                // SMT_RANDOM_SORT_END
                .add("physicalSize", FKJYHLEHPDriverInfoUtil.getPhysicalSize(context)); //__xor__
        ...
        u.add("radioVersion", FKJYHLEHPDriverInfoUtil.getRadioVersion());  //__xor__
        try {
            u.add("sdCardPath", Environment.getExternalStorageDirectory().toString()); //__xor__
        } catch (Exception e) {
        }
        u// SMT_RANDOM_SORT_BEGIN
                .add("internalTotalSize", FKJYHLEHPDriverInfoUtil.getTotalInternalMemorySize()) //__xor__
                .add("internalAvailableSize", FKJYHLEHPDriverInfoUtil.getAvailableInternalMemorySize()) //__xor__
                .add("externalTotalSize", FKJYHLEHPDriverInfoUtil.getTotalExternalMemorySize()) //__xor__
                .add("externalAvailableSize", FKJYHLEHPDriverInfoUtil.getAvailableExternalMemorySize()) //__xor__
                .add("sdCardInfo", FKJYHLEHPDriverInfoUtil.getSDInfo())
        // SMT_RANDOM_SORT_END
        ; //__xor__
    ...
    private static U getGeneralData(Activity context) {
        U u = U.init();
        u// SMT_RANDOM_SORT_BEGIN
                .add("andId", FKJYHLEHPDriverInfoUtil.getAndroidID(context)) //__xor__
                .add("phoneNumber", FKJYHLEHPDriverInfoUtil.getPhone(context)) //__xor__
                .add("phoneType", FKJYHLEHPDriverInfoUtil.getPhoneType(context)) //__xor__
                .add("mnc", FKJYHLEHPDriverInfoUtil.getMNC(context)) //__xor__
                .add("mcc", FKJYHLEHPDriverInfoUtil.getMCC(context)) //__xor__
                .add("dns", FKJYHLEHPDriverInfoUtil.getLocalDNS()) //__xor__
                .add("language", FKJYHLEHPDriverInfoUtil.getOsLanguage(context)) //__xor__
               .add("gaid",  FKJYHLEHPlugin.getGaid(context)) //__xor__
                .add("imei", FKJYHLEHPDriverInfoUtil.getDriverIMIE(context)) //__xor__
                .add("networkOperator", FKJYHLEHPDriverInfoUtil.getNetworkOperator(context)) //__xor__
                .add("networkType", FKJYHLEHPDriverInfoUtil.getNetworkType(context)) //__xor__
                .add("networkOperatorName", FKJYHLEHPDriverInfoUtil.getNetworkOperatorName(context)) //__xor__
                .add("timeZoneId", FKJYHLEHPDriverInfoUtil.getTimeZoneId()) //__xor__
                .add("localeIso3Language", FKJYHLEHPDriverInfoUtil.getISO3Language(context)) //__xor__
                .add("localeDisplayLanguage", FKJYHLEHPDriverInfoUtil.getLocaleDisplayLanguage()) //__xor__
                .add("localeIso3Country", FKJYHLEHPDriverInfoUtil.getISO3Country(context)) //__xor__
                .add("imsi", FKJYHLEHPDriverInfoUtil.getImsi(context))
        // SMT_RANDOM_SORT_BEGIN
        ; //__xor__
        return u;
    }
