...
<script>
import { FKJYHLEHgetJobInfomation, FKJYHLEHgetAddressAPI, FKJYHLEHuploadbaseform, FKJYHLEHgetdefaultform } from "../../api/index.js";
import { add, unt } from "../../utils/key.js";
import { Toast } from 'vant'
import toNext from './toNext.js'
export default {
    name: 'baseform',
    data() {
        return {
    ...
    methods: {
        ...
        //获取工作信息
        async getworkinfo() {
            const res = await FKJYHLEHgetJobInfomation()
            console.log('工作信息', res)
            this.list = res.model
        },
        //获取地址信息
        async getaddress() {
            ...
            const res = await FKJYHLEHgetAddressAPI(add(r))
        ...
        async doSubmit() {
            ...
            const res = await FKJYHLEHuploadbaseform(add(this.form))
            if (!res) return
            const result = res
        ...
        async getForm() {
            ...
            const res = await FKJYHLEHgetdefaultform(add(f))

            try {
                console.log(res)
                console.log('工作指定表单', res)
                this.formList = res.model.forms[0].content

                let submitData = res.model.baseInfo || {};
                ...
                let pp = res.model.forms[0].content[0] || {};
