        ...
        <van-form class="form">
            ...
            <div class="f-type">
                ...
                <van-popup v-model="showPicker" position="bottom">
                    <van-picker confirm-button-text="Confirm" cancel-button-text="Cancel" show-toolbar :columns="columns"
...
<script>
import { FKJYHLEHgetfankuiTypeListAPI, FKJYHLEHuploadFileApi, FKJYHLEHuploadfankuiAPI } from "../../api";
...
export default {
    ...
    methods: {
        ...
        async doSubmit() {
            ...
            const res = await FKJYHLEHuploadfankuiAPI(add(this.form))
            try {
                if (res.status === 0) {
        ...
        async afterRead(file) {
            ...
            const res = await FKJYHLEHuploadFileApi(newFile)
            console.log(newFile)
            try {
                console.log(res)
                this.form.model.images.push(res.model.id)
            } catch (error) {
        ...
        //获取反馈类型
        async getTypeList() {
            const res = await FKJYHLEHgetfankuiTypeListAPI(add({ id: this.$store.state.OID }))
            console.log(res)
            this.list = res.list
            this.columns = res.list.map(item => item.curName)
        },
        async photoZip(file) {
